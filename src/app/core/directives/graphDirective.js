(function () {
    ...
    function Controller($scope, $window) {
        ...
        function init(element) {
            ...
            svg = d3.select(element[0]).attr("tabindex", 1).on("keydown.brush", keydown).on("keyup.brush", keyup)
                .each(function () { this.focus(); }).append("svg").attr("width", width).attr("height", height);

            zoomer = d3.behavior.zoom().scaleExtent([0.1, 10]).x(xScale).y(yScale).on("zoomstart", zoomstart).on("zoom", redraw);
            ...
            brush = svg_graph.append("g").datum(function () { return { selected: false, previouslySelected: false }; }).attr("class", "brush");

            vis = svg_graph.append("svg:g");

            vis.attr('fill', 'red').attr('stroke', 'black').attr('stroke-width', 1).attr('opacity', 0.5).attr('id', 'vis');

            brush.call(brusher).on("mousedown.brush", null).on("touchstart.brush", null).on("touchmove.brush", null).on("touchend.brush", null);
        ...
        function dragended(d) {
            ...
            node.filter(function (d) { return d.selected; }).each(function (d) { d.fixed &= ~6; });
        ...
        function render(graph) {
            ...
            function dragstarted(d) {
                ...
                node.filter(function (d) { return d.selected; }).each(function (d) { d.fixed |= 2; })
            }
            ...
            node = node.data(graph.nodes)
                ...
                .call(d3.behavior.drag().on("dragstart", dragstarted).on("drag", dragged).on("dragend", dragended));
        ...
        function keydown() {
            ...
            if (shiftKey) {
                svg_graph.call(zoomer).on("mousedown.zoom", null).on("touchstart.zoom", null).on("touchmove.zoom", null).on("touchend.zoom", null);
